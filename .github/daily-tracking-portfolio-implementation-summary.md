...
- do not change anything related to recommend_model.py or run_monte_carlo.py unless it is tested rigorously to provide the same result as it does now. This includes functions/MonteCarloBacktest.py and model_config_loader.py
...
[x] Create the central JSON configuration file at:
   `/Users/donaldpg/pyTAAA_data/naz100_sp500_abacus/pytaaa_naz100_sp500_abacus.json`

[x] Create local enhanced configuration:
   `abacus_combined_PyTAAA_status.params.json`

[x] Ensure JSON contains all required sections:
...
[x] Set up `model_choices` in the JSON uses template-based paths:
```json
"model_choices": {
  "cash": "",
  "naz100_hma": "{base_folder}/naz100_hma/data_store/{data_file}",
...
[ ] Implement `daily_abacus_update.py` with these core functions:
...
[ ] Add command-line interface with `--json` parameter for configuration file

[ ] Integrate with existing `run_pytaaa()` function for actual updates

### Step 4: Configure Active Model Detection
[ ] Ensure holdings file contains `trading_model:` line at the end
[ ] Configure model choices in JSON with template-based paths
[ ] Set up automatic detection and path resolution based on active model

### Step 5: Set Up Data Stores
[x] Create data store directories for each model:
...
[ ] Create abacus tracking data store:
   - `/Users/donaldpg/pyTAAA_data/naz100_sp500_abacus/data_store/`

[ ] Initialize required parameter files in naz100_sp500_abacus data store:
   - `PyTAAA_status.params`
   - `PyTAAA_holdings.params` (with trading_model tag for abacus)
   - `PyTAAA_ranks.params`

### Step 6: Configure Web Output
[x] Set up web output directory: `/Users/donaldpg/pyTAAA_data/naz100_sp500_abacus/pyTAAA_web`
[ ] Configure HTML template generation for dashboard
[ ] Set up automatic web file updates with current portfolio data

### Step 7: Testing and Validation
[ ] Test active model detection with different trading models
[ ] Verify configuration loading and validation
[ ] Test daily updates with --verbose flag
[ ] Validate web content generation
[ ] Test integration with Monte Carlo and recommendation systems
...
### Potentially Later - don't implement now -- Cron Job Setup 
```bash
