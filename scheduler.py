...
class Scheduler(Thread):
    ...
    def halt(self):
        ...
        list(map(self.drop, list(self.tasks.keys())))
...
class RunUntilSuccess(object):
    ...
    def __call__(self):
        try_count = 0
        is_success = False
        #print "is_success, try_count, self.num_tries = ", is_success, try_count, self.num_tries
        while not is_success and try_count < self.num_tries:
            try_count += 1
            try:
                #print "here's the passed func (as a string) = ", str(self.func())
