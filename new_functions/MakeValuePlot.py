...
def makeTrendDispersionPlot( ):
    ...
    figure5path = 'PyTAAADL_backtestWithTrend.png'  # re-set to name without full path
    figure5_htmlText = "\n<br><h3>DL Monthly backtest</h3>\n"
    figure5_htmlText = figure5_htmlText + "\nMost recent backtest for PyTAAADL. PyTAAADL combines multiple trained DL models based on recent performance metrics. These metrics are computed conventionally, not using DL. Choices are made from these metrics to choose the number of stocks to hold, and what performance metric is used to judge recent performance.\n"
    figure5_htmlText = figure5_htmlText + '''<br><img src="'''+figure5path+'''" alt="PyTAAADL by DonaldPG" width="850" height="500"><br>\n'''
