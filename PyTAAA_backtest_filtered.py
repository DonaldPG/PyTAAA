...
filename = os.path.join( os.getcwd(), 'symbols', 'Naz100_Symbols.txt' )                   # plotmax = 1.e10, runnum = 902

json_fn = '/Users/donaldpg/pytaaa_data/Naz100_fsma/pytaaa_Naz100_fsma.json'
...
outfiledir = "/Users/donaldpg/pyTAAA_data/naz100_fsma/data_store/pngs"
outfilename = os.path.join(
    outfiledir,
    "Naz100-fSMAs_montecarlo_"+str(dateForFilename)+"_"+str(runnum)+".csv"
)
...
for iter in range(randomtrials):
    ...
    MA1 = int(random.uniform(15,250)+.5) // 2
    ...
    if iter >= randomtrials / runs_fraction :
        ...
        monthsToHold = 1
        LongPeriod = 256
        stddevThreshold = 3.97
        MA1 = 115
        MA2 = 19
        MA3 = 20
        sma2factor = .899
        rankThresholdPct = .0815
        riskDownside_min = .2935
        riskDownside_max = 4.64
        '''
    ...
    if iter < randomtrials / runs_fraction:
        ...
        # lo_factor2, hi_factor2 = 0.8, 1.25
        # numberStocksTraded = choice([5,6,7,8])
        # monthsToHold = choice([1,1,1,1,1,1,1,1,1,2])
        # LongPeriod = int( 256 * random.uniform(lo_factor, hi_factor) )
        # stddevThreshold = stddevThreshold * random.uniform(lo_factor, hi_factor)
        # MA1 = int( 116 * random.uniform(lo_factor, hi_factor) )
        # MA2 = int( 19 * random.uniform(lo_factor, hi_factor) )
        # MA2offset = choice([1,2,3])
        # MA3 = int( 21 * random.uniform(lo_factor, hi_factor) )
        # sma2factor = .899 * random.uniform(lo_factor2, hi_factor2)
        # rankThresholdPct = .0815 * random.uniform(lo_factor2, hi_factor2)
        # riskDownside_min = .2935 * random.uniform(lo_factor2, hi_factor2)
        # riskDownside_max = 4.64 * random.uniform(lo_factor2, hi_factor2)

        lo_factor, hi_factor = 0.65, 1.5
        lo_facto, hi_factor2 = 0.8, 1.25
        numberStocksTraded = choice([5,6,6,7,7,8,8])
        monthsToHold = choice([1,1,1,1,1,1,1,1,1,2])
        # LongPeriod = int(random_triangle(low=168, mid=475, high=650, size=1))
        ...
        # MA1 = int(random_triangle(low=110, mid=215, high=290, size=1))
        # MA2 = int(random_triangle(low=7, mid=20, high=39, size=1))
        # MA2offset = choice([1,1,1,2,2,3])
        # MA3 = int( 22 * random.uniform(lo_factor, hi_factor) )
        MA1 = int(random_triangle(low=110, mid=151, high=200, size=1))
        MA2 = int(random_triangle(low=10, mid=15, high=24, size=1))
        MA2offset = choice([1,1,1,2,2,3,4,5])
        MA3 = int( 22 * random.uniform(lo_factor, hi_factor) )
    ...
    ########################################################################
    # print(" ... Start computing Hull MAs. adjClose.shape = " + str(adjClose.shape))
    print(" ... Start computing filtered SMAs. adjClose.shape = " + str(adjClose.shape))

    txt = "   . SMA/fSMA/HMA periods: " + str((MA2, MA2 + MA2offset, MA1)) + " "
    print(txt, end='')
    # sma0 = hma( adjClose, MA2 )               # MA2 is shortest
    sma0 = SMA_filtered_2D(adjClose, MA2, sma_filt_val)               # MA2 is shortest

    txt = txt + "."
    print("\r" + txt, end='')
    # sma1 = hma( adjClose, MA2 + MA2offset )
    sma1 = SMA_filtered_2D(adjClose, MA2 + MA2offset, sma_filt_val)               # MA2 is shortest

    txt = txt + "."
    print("\r" + txt, end='')
    # sma2 = sma2factor * hma( adjClose, MA1 )  # MA1 is longest
    sma2 = sma2factor * SMA_filtered_2D(adjClose, MA1, sma_filt_val)  # MA1 is longest
    txt = txt + "."
    print("\r" + txt, end='')

    print("\n ... Start computing signal2D")
    ...
    print(" signal2D check (isnans): ",signal2D[np.isnan(signal2D)].shape)
    ...
    print(" 1 - monthvalue check: ",monthvalue[np.isnan(monthvalue)].shape)
    ...
    plt.text(50,10.**(np.log10(7000)+(.91*plotrange)),'15 Yr '+fSharpe15Yr+'  '+fReturn15Yr+'  '+fDrawdown15Yr,fontsize=8)
    plt.text(50,10.**(np.log10(7000)+(.87*plotrange)),'10 Yr '+fSharpe10Yr+'  '+fReturn10Yr+'  '+fDrawdown10Yr,fontsize=8)
    plt.text(50,10.**(np.log10(7000)+(.83*plotrange)),' 5 Yr  '+fSharpe5Yr+'  '+fReturn5Yr+'  '+fDrawdown5Yr,fontsize=8)
    plt.text(50,10.**(np.log10(7000)+(.79*plotrange)),' 3 Yr  '+fSharpe3Yr+'  '+fReturn3Yr+'  '+fDrawdown3Yr,fontsize=8)
    plt.text(50,10.**(np.log10(7000)+(.75*plotrange)),' 2 Yr  '+fSharpe2Yr+'  '+fReturn2Yr+'  '+fDrawdown2Yr,fontsize=8)
    plt.text(50,10.**(np.log10(7000)+(.71*plotrange)),' 1 Yr  '+fSharpe1Yr+'  '+fReturn1Yr+'  '+fDrawdown1Yr,fontsize=8)
    ...
    # plt.text(2250,1000*10**(.95*plotrange),'Period Sharpe AvgProfit  Avg DD',fontsize=7.5,color='b')
    # plt.text(2250,1000*10**(.91*plotrange),'15 Yr '+fVSharpe15Yr+'  '+fVReturn15Yr+'  '+fVDrawdown15Yr,fontsize=8,color='b')
    # plt.text(2250,1000*10**(.87*plotrange),'10 Yr '+fVSharpe10Yr+'  '+fVReturn10Yr+'  '+fVDrawdown10Yr,fontsize=8,color='b')
    # plt.text(2250,1000*10**(.83*plotrange),' 5 Yr  '+fVSharpe5Yr+'  '+fVReturn5Yr+'  '+fVDrawdown5Yr,fontsize=8,color='b')
    # plt.text(2250,1000*10**(.79*plotrange),' 3 Yr  '+fVSharpe3Yr+'  '+fVReturn3Yr+'  '+fVDrawdown3Yr,fontsize=8,color='b')
    # plt.text(2250,1000*10**(.75*plotrange),' 2 Yr  '+fVSharpe2Yr+'  '+fVReturn2Yr+'  '+fVDrawdown2Yr,fontsize=8,color='b')
    # plt.text(2250,1000*10**(.71*plotrange),' 1 Yr  '+fVSharpe1Yr+'  '+fVReturn1Yr+'  '+fVDrawdown1Yr,fontsize=8,color='b')
    plt.text(2250,10.**(np.log10(7000)+(.95*plotrange)),'Period Sharpe AvgProfit  Avg DD',fontsize=7.5,color='b')
    plt.text(2250,10.**(np.log10(7000)+(.91*plotrange)),'15 Yr '+fVSharpe15Yr+'  '+fVReturn15Yr+'  '+fVDrawdown15Yr,fontsize=8,color='b')
    plt.text(2250,10.**(np.log10(7000)+(.87*plotrange)),'10 Yr '+fVSharpe10Yr+'  '+fVReturn10Yr+'  '+fVDrawdown10Yr,fontsize=8,color='b')
    plt.text(2250,10.**(np.log10(7000)+(.83*plotrange)),' 5 Yr  '+fVSharpe5Yr+'  '+fVReturn5Yr+'  '+fVDrawdown5Yr,fontsize=8,color='b')
    plt.text(2250,10.**(np.log10(7000)+(.79*plotrange)),' 3 Yr  '+fVSharpe3Yr+'  '+fVReturn3Yr+'  '+fVDrawdown3Yr,fontsize=8,color='b')
    plt.text(2250,10.**(np.log10(7000)+(.75*plotrange)),' 2 Yr  '+fVSharpe2Yr+'  '+fVReturn2Yr+'  '+fVDrawdown2Yr,fontsize=8,color='b')
    plt.text(2250,10.**(np.log10(7000)+(.71*plotrange)),' 1 Yr  '+fVSharpe1Yr+'  '+fVReturn1Yr+'  '+fVDrawdown1Yr,fontsize=8,color='b')
    ...
    ###
    try:
        # filepath = os.path.join( os.getcwd(), "pyTAAAweb_HMAbacktestPortfolioValue.params" )
        plot_fn = os.path.join(
            outfiledir,
