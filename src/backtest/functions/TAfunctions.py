...
def percentileChannel_2D(x,minperiod,maxperiod,incperiod,lowPct,hiPct):
    ...
    print(
        " ... inside percentileChannel_2D ...  minperiod,maxperiod,incperiod, = " + \
        str((minperiod,maxperiod,incperiod,))
    )
...
#----------------------------------------------
def percentileChannel_2D_optimized(x, minperiod, maxperiod, incperiod, lowPct, hiPct, verbose=False):
    """
    Optimized version of percentileChannel_2D with vectorized operations.
    
    Performance improvements:
    1. Pre-compute period ranges once instead of repeatedly
    2. Use vectorized percentile computation with axis parameter
    3. Batch process time windows to reduce loop overhead
    4. Use views instead of slices where possible
    5. Avoid redundant computations
    
    Estimated speedup: 5-10x for SP500 (500 stocks), even more for larger datasets
    """
    if verbose:
        print(" ... inside percentileChannel_2D_optimized ...  x min,mean,max = ", 
              x.min(), x.mean(), x.max())
        print(" ... minperiod,maxperiod,incperiod = ", minperiod, maxperiod, incperiod)
    ...
    # Pre-allocate temporary storage for all periods at once
    # Shape: (num_periods, num_stocks)
    temp_min = np.zeros((num_periods, num_stocks), dtype=float)
    temp_max = np.zeros((num_periods, num_stocks), dtype=float)
    ...
    for i in range(num_days):
        ...
        for j, period in enumerate(periods):
            ...
            if i < minx:
                ...
                # Compute percentiles for all stocks at once (vectorized)
                # This is the key optimization - one np.percentile call for all stocks
                window_data = x[:, minx:i+1]
                
                if window_data.shape[1] > 0:
                    temp_min[j] = np.percentile(window_data, lowPct, axis=1)
                    temp_max[j] = np.percentile(window_data, hiPct, axis=1)
                    valid_periods += 1
                else:
                    temp_min[j] = x[:, i]
                    temp_max[j] = x[:, i]
        ...
        if valid_periods > 0:
            minchannel[:, i] = np.mean(temp_min[:valid_periods], axis=0)
            maxchannel[:, i] = np.mean(temp_max[:valid_periods], axis=0)
    
    if verbose:
        print(" lowPct,hiPct = ", lowPct, hiPct)
        print(" x min,mean,max = ", x.min(), x.mean(), x.max())
        print(" computed channels for ", num_days, " days")
...
def computeSignal2D( adjClose, gainloss, params ):
    ...
    narrowDays = params.get('narrowDays', [params.get('narrowDays_min', 5), params.get('narrowDays_max', 7)])
    mediumDays = params.get('mediumDays', [params.get('mediumDays_min', 20), params.get('mediumDays_max', 30)])
    wideDays = params.get('wideDays', [params.get('wideDays_min', 60), params.get('wideDays_max', 100)])

    lowPct = float(params['lowPct'])
    hiPct = float(params['hiPct'])
    sma2factor = float(params.get('MA2factor', params.get('sma2factor', 2.0)))
...
#     # if stock has nan price at the end, set deltarank to zero so stock will not be chosen
#     for ii in range(monthgainloss.shape[0]):
...
#                 print(
#                     "*******setting delta (Rank) low... " +\
#                     "Stock has nan price at end... ",
#                     ii, symbols[ii]
#                 )
#     print("TAFunctions.sharpeWeightedRank_2D ... nan filter completed ...")
...
def sharpeWeightedRank_2D(
    ...
    try:
        ...
        import scipy.stats.mstats as bn
    # from functions.quotes_for_list_adjClose import get_Naz100List, get_SP500List, get_ETFList
    from src.backtest.functions.GetParams import get_json_params, get_symbols_file
    ...
    print("   ... inside function TAFunctions.sharpeWeightedRank_2D ...")
    ...
    signal2D *= signal2D.max()
    if signal2D.max() == 0:
        signal2D = np.ones_like(signal2D)
    ...
    print(" ... currentSymbolList = ", currentSymbolList)
    ...
    if makeQCPlots==True:
        ...
        peList = []
        floatPE_list = []
        if "temp_realization" not in json_fn:
            #quote_df = getQuote(symbols)
            for ticker in symbols:
                floatPE_list.append( get_pe(ticker) )
                peList.append( str(floatPE_list[-1]) )
            ...
            # Skip P/E fetching in Monte Carlo mode
            peList = ["N/A"] * len(symbols)
            floatPE_list = [np.nan] * len(symbols)
            print(" ... Skipping P/E ratios download in Monte Carlo mode...")
